...
# +
path = os.path.join('..', 'VLE_240', 'prod_vle')
...
frames = [os.path.join('VLE', 'profiles', '{:s}_nr_dens.png'.format(file.split(os.sep)[1])) for file in files]
print(frames)
with imageio.get_writer('VLE/profiles.gif', mode='I') as writer:
...
def fit_water_density(file, box_size=40, side='lhs', plot=False):
    ...
    p, pcov = curve_fit(lambda x, x0, d: water_density(x, x0, d, liq_density, vap_density, side=side),
                    density[this_slice, 0], density[this_slice, indices.get('Water')], p0, bounds=([-box_size/2., 1e-4], [box_size/2, 5]))
